...
const VOICE_BUBBLE_EDGE_OUTPUT_FORMAT = "webm-24khz-16bit-mono-opus";
...
function resolveEdgeOutputFormat(config: ResolvedTtsConfig, channelId?: string | null): string {
  const configured = config.edge.outputFormat;
  if (!channelId || !VOICE_BUBBLE_CHANNELS.has(channelId)) {
    return configured;
  }

  // Feishu/Telegram/WhatsApp voice bubbles expect Opus-compatible audio.
  const normalized = configured.trim().toLowerCase();
  if (normalized.includes("opus") || normalized.includes("ogg") || normalized.includes("webm")) {
    return configured;
  }
  return VOICE_BUBBLE_EDGE_OUTPUT_FORMAT;
}

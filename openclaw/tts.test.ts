...
describe("tts", () => {
  ...
  describe("resolveEdgeOutputFormat", () => {
    ...
    it("forces opus-friendly edge output for voice-bubble channels when config is mp3", () => {
      const config = resolveTtsConfig(baseCfg);
      expect(resolveEdgeOutputFormat(config, "feishu")).toBe("webm-24khz-16bit-mono-opus");
      expect(resolveEdgeOutputFormat(config, "telegram")).toBe("webm-24khz-16bit-mono-opus");
      expect(resolveEdgeOutputFormat(config, "whatsapp")).toBe("webm-24khz-16bit-mono-opus");
      expect(resolveEdgeOutputFormat(config, "discord")).toBe("audio-24khz-48kbitrate-mono-mp3");
    });

    it("keeps configured opus/webm edge output for voice-bubble channels", () => {
      const opusCfg = resolveTtsConfig({
        ...baseCfg,
        messages: {
          tts: {
            edge: { outputFormat: "webm-24khz-16bit-mono-opus" },
          },
        },
      } as OpenClawConfig);
      expect(resolveEdgeOutputFormat(opusCfg, "feishu")).toBe("webm-24khz-16bit-mono-opus");

      const oggCfg = resolveTtsConfig({
        ...baseCfg,
        messages: {
          tts: {
            edge: { outputFormat: "ogg-24khz-16bit-mono-opus" },
          },
        },
      } as OpenClawConfig);
      expect(resolveEdgeOutputFormat(oggCfg, "feishu")).toBe("ogg-24khz-16bit-mono-opus");
    });
  });

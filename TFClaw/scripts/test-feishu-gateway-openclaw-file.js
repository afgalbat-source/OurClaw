...
function parseArgs(argv) {
  const args = {
    ...
    prompt:
      "请严格执行：先创建一个txt文件，内容为 TFClaw file e2e test；然后把该txt文件作为附件发送给我。最终不要只发文字。",
    chatType: "p2p",
    senderOpenId: "",
    timeoutMs: 180_000,
    pollMs: 2_500,
...
async function main() {
  ...
  const fakeInboundEvent = {
    header: {
      event_id: `openclaw-file-test-${Date.now()}`,
    },
    event: {
      sender: {
        sender_id: {
  ...
  while (Date.now() < deadline) {
    ...
    const hasFile = fresh.some((item) => item.msgType === "file");
    const hasAnyReply = fresh.some((item) => item.msgType !== "text");
    if ((args.requireFile && hasFile) || (!args.requireFile && hasAnyReply)) {
      console.log("RESULT: PASS");
      console.log(`fresh_message_count: ${fresh.length}`);
      console.log(`fresh_file_count: ${fresh.filter((item) => item.msgType === "file").length}`);

...
function buildScopedTmuxSocketPath(linuxUser: string): string {
  const rawRoot = (process.env.TFCLAW_TMUX_USER_SOCKET_ROOT ?? "/tmp").trim();
  const rootDir = rawRoot && path.isAbsolute(rawRoot) ? rawRoot : "/tmp";
  const tokenPart = sanitizeTmuxName((TOKEN ?? "token").slice(0, 8)) || "token";
  const userPart = linuxUser.trim().replace(/[^A-Za-z0-9_-]/g, "_").slice(0, 32) || "user";
  return path.join(rootDir, `tfclaw-${tokenPart}-${userPart}.sock`);
}
...
function loadRelayConfig(): { configPath: string; token: string; url: string } {
  const configPath = path.resolve(process.env.TFCLAW_CONFIG_PATH ?? "config.json");
  let rawConfig: Record<string, unknown> = {};
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      console.warn(`[relay] failed to parse config file (${configPath}): ${msg}`);
    }
  }

  const relay = toObject(rawConfig.relay);
  return {
    configPath,
    token: toString(relay.token).trim(),
    url: toString(relay.url).trim(),
  };
}
...
const RELAY_CONFIG = loadRelayConfig();
const TOKEN = (process.env.TFCLAW_TOKEN ?? RELAY_CONFIG.token).trim();
const RELAY_URL = (process.env.TFCLAW_RELAY_URL ?? RELAY_CONFIG.url).trim();
...
if (!TOKEN) {
  console.error(`Missing TFCLAW_TOKEN. Set relay.token in ${RELAY_CONFIG.configPath} or TFCLAW_TOKEN in env.`);
  process.exit(1);
}

if (!RELAY_URL) {
  console.error(`Missing TFCLAW_RELAY_URL. Set relay.url in ${RELAY_CONFIG.configPath} or TFCLAW_RELAY_URL in env.`);
  process.exit(1);
}
...
function parseTmuxControlExecContext(sessionKeyRaw?: string): TmuxControlExecContext {
  ...
  return {
    linuxUser,
    homeDir,
    forcedSocketPath: buildScopedTmuxSocketPath(linuxUser),
  };
}
